...
*** TODO Training Strategy
    ...
    The agent needs to predict the future rewards of all the 3 allowed actions, turning left, turning right and going forward.
    The prediction function is called an action value function, and denoted as 
    $Q(s,a)$, where $s$ and $a$ are the state and action.
    In our program, the action value function is a neural network (Figure [[fig:nn_2d]]).
    The neural network takes the state as its input. 
    Its output layer contains 3 units, corresponding to the 3 allowed actions.
    These outputs are regarded as the predicted action value of each action.
    ...
    The weights of the neural connections are randomized by Gaussian Distribution 
    ${\mathcal {N}}(0 ,\,0.005 ^{2})$,
    and the biases of neural units are set to 0.
