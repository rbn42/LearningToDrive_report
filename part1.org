#+OPTIONS: H:7
     We use Panda3D to generate the 
...
**** Collision Detection
     We use a physics engine Bullet to detect collisions and send signals to the agent.
     This is one of the physics engine integrated with Panda3D.
     A physics engine is a simulation software for physical systems. 
     For example, in rigid body dynamics, 
     the engine detects collisions of different objects, 
     and calculates the results of the collisions.
     But in this experiments, only the collision detection function is required.
*** Training
**** Random Actions
